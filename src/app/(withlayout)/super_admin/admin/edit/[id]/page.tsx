...
const EditAdminPage = ({ params }: any) => {
  const { data: adminData, isLoading: adminDataLoading } = useAdminQuery(
    params?.id
  );
  const { data: departmentsData, isLoading: departmentsDataLoading } =
    useDepartmentsQuery({ limit: 100, page: 1 });

  const [updateAdmin] = useUpdateAdminMutation();

  const departments = departmentsData?.departments as IDepartment[];
  ...
  return (
    <>
      {adminDataLoading || departmentsDataLoading ? (
        <LoadingSpinner />
      ) : (
        <div>
          <UMBreadCrumb
          ...
          />
          <h1 style={{ margin: "12px 0px" }}>Edit Admin {params?.id}</h1>
